...
export interface SelectProps<TOption extends SimpleSelectOption> {
  label: string;
  required?: boolean;
  value: string;
  options: TOption[];
...
/**
 * Generic searchable single-select combobox. Promoted from the IM module's
 * FormSelectField so other modules (PM) can reuse it without depending on
 * IM-local types.
 */
export function Select<TOption extends SimpleSelectOption>({
  label,
  required = false,
  value,
  options,
